...
// ==============================================
// SCROLL INFINITO PARA "TODOS LOS PRODUCTOS" - COMPLETO
// ==============================================

// Configuración para scroll infinito
const SCROLL_INFINITO_CONFIG = {
...
// NUEVA: Función optimizada para mostrar TODOS los productos
async function mostrarTodosLosProductosCompletoOptimizado() {
    try {
        console.log('🔄 Iniciando carga optimizada de todos los productos...');
        
        // 1. Cambiar a vista correcta
        ...
        SCROLL_INFINITO_CONFIG.paginaActual = 0;
        SCROLL_INFINITO_CONFIG.cargando = false;
        SCROLL_INFINITO_CONFIG.finDeProductos = false;
        SCROLL_INFINITO_CONFIG.totalProductos = productosGlobal.length;
        
        // 3. Mostrar skeletons iniciales
        const grid = document.getElementById('grid-todos');
        grid.innerHTML = `
        ...
        // 5. Esperar breve momento para mejor UX
        setTimeout(async () => {
        ...
        console.error('❌ Error en mostrarTodosLosProductosCompletoOptimizado:', error);
    }
}

// NUEVA: Cargar primera página
async function cargarPrimeraPagina() {
    if (SCROLL_INFINITO_CONFIG.cargando) return;
    
    SCROLL_INFINITO_CONFIG.cargando = true;
    
    try {
        const inicio = SCROLL_INFINITO_CONFIG.paginaActual * SCROLL_INFINITO_CONFIG.productosPorPagina;
        const fin = inicio + SCROLL_INFINITO_CONFIG.productosPorPagina;
        ...
        if (productosPagina.length === 0) {
            ...
            SCROLL_INFINITO_CONFIG.finDeProductos = true;
            return;
        }
        ...
        // Actualizar contador
        const mostrados = productosPagina.length;
        const total = productosGlobal.length;
        contador.textContent = `${mostrados} de ${total} productos`;
        
        // Verificar si hay más productos
        SCROLL_INFINITO_CONFIG.paginaActual++;
        SCROLL_INFINITO_CONFIG.finDeProductos = fin >= productosGlobal.length;
        
        // Mostrar botón "Ver más" si hay más productos
        if (!SCROLL_INFINITO_CONFIG.finDeProductos) {
...
async function cargarSiguientePagina() {
    if (SCROLL_INFINITO_CONFIG.cargando || SCROLL_INFINITO_CONFIG.finDeProductos) return;
    ...
    try {
        ...
        // Agregar productos al final del grid
        const htmlProductos = productosPagina.map(producto => crearCardProductoHTML(producto)).join('');
        grid.innerHTML += htmlProductos;
        
        // Actualizar contador
        const mostrados = Math.min(fin, productosGlobal.length);
        const total = productosGlobal.length;
        contador.textContent = `${mostrados} de ${total} productos`;
        ...
        if (SCROLL_INFINITO_CONFIG.finDeProductos) {
            ...
            const restantes = productosGlobal.length - fin;
            document.getElementById('btn-ver-mas').textContent = 
...
// NUEVA: Configurar observador de scroll
function configurarObservadorScroll() {
    // Eliminar observador anterior si existe
    if (SCROLL_INFINITO_CONFIG.observador) {
        SCROLL_INFINITO_CONFIG.observador.disconnect();
    }
    
    // Crear nuevo observador
    SCROLL_INFINITO_CONFIG.observador = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting && 
                !SCROLL_INFINITO_CONFIG.cargando && 
                !SCROLL_INFINITO_CONFIG.finDeProductos) {
    ...
    if (elementoCargando) {
        SCROLL_INFINITO_CONFIG.observador.observe(elementoCargando);
    }
}

// NUEVA: Función para botón "Ver más" manual
function cargarMasProductosManual() {
    if (!SCROLL_INFINITO_CONFIG.cargando && !SCROLL_INFINITO_CONFIG.finDeProductos) {
        cargarSiguientePagina();
    }
}

// ==============================================
// REEMPLAZAR TU FUNCIÓN ACTUAL
// ==============================================

// Reemplaza tu función actual de mostrarTodosLosProductosCompleto
window.mostrarTodosLosProductosCompleto = mostrarTodosLosProductosCompletoOptimizado;

// Reemplaza tu función actual de cargarMasProductos
window.cargarMasProductos = cargarMasProductosManual;

// ==============================================
// MEJORAR LA FUNCIÓN cargarPorTipo('TODOS')
// ==============================================

// Modificar tu función cargarPorTipo para manejar 'TODOS'
async function cargarPorTipoModificada(tipo) {
    console.log('📁 Cargando tipo:', tipo);
    
    if (tipo === 'TODOS') {
        // Usar la nueva función optimizada
        await mostrarTodosLosProductosCompletoOptimizado();
        mostrarCategoriasNivel0();
        return;
    }

    // Resto de tu código para otros tipos...
    cambiarAVista('productos');
    // ... (mantener el resto de tu código)
}

// Sobrescribir tu función original
window.cargarPorTipo = cargarPorTipoModificada;
